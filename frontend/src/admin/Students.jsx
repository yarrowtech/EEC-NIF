...
const Students = ({ setShowAdminHeader }) => {
  ...
  // ✅ SAFE filter (name/roll/email might be missing)
  const filteredStudents = studentData.filter((student) => {
    const name = (student.name || "").toLowerCase();
    const roll = student.roll ? String(student.roll) : "";
    const email = (student.email || "").toLowerCase();
    const term = searchTerm.toLowerCase();
    return (
      name.includes(term) || roll.includes(searchTerm) || email.includes(term)
    );
  });
  ...
  // Emotional Wellbeing Functions
  const getWellbeingStatus = (studentId) => {
    if (!studentId) return { mood: "neutral", socialEngagement: 5, academicStress: 5, behaviorChanges: false };
  ...
  // ✅ NIF: fetch NIF students + courses
  useEffect(() => {
    setShowAdminHeader(true);
    ...
    const fetchData = async () => {
      try {
        ...
        if (!resStudents.ok) {
          throw new Error("Failed to fetch NIF students");
        }
        const students = await resStudents.json();
        setStudentData(students || []);

        // 🔹 Fetch courses (if you use them elsewhere)
        ...
        console.error("Error fetching NIF students or courses:", err);
      }
    };
  ...
  // ✅ NIF: create student in NifStudent collection
  const handleAddStudentSubmit = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/nif/students`,
        {
          ...
          body: JSON.stringify({
            ...newStudent,
            academicYear: "2025-26", // later make dynamic if needed
          }),
        }
      );
      ...
      const data = await res.json();
      if (!res.ok) {
        console.error("NIF registration failed:", data);
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Manage and monitor NIF student information, health status,
              attendance, and fees
            </p>
          </div>
          <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">
                  {studentData.filter((student) => {
                    ...
                    const wellbeing = getWellbeingStatus(key);
                    return (
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              ...
                              >
                                {wellbeing.mood
                                  .charAt(0)
                                  .toUpperCase() + wellbeing.mood.slice(1)}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 text-xs text-gray-500">
                              <span>Stress: {wellbeing.academicStress}/10</span>
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {(() => {
                      const key = selectedStudent._id || selectedStudent.id;
                      const wellbeing = getWellbeingStatus(key);
                      const moodConfig = getMoodIcon(wellbeing.mood);
                      const MoodIcon = moodConfig.icon;
                      return (
              ...
              <div className="p-6 border-b border-gray-200">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-gray-600 mt-1">
                      Fill in the student details and select a program
                    </p>
                  </div>
                  <button
                    onClick={() => setShowAddForm(false)}
